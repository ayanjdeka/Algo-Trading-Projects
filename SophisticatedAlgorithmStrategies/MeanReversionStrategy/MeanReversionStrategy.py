...
orders = [] #tracking buy/sell order, +1 for a buy order, and -1 for a sell order
positions = [] #tracks positions, +ve for long positions, -ve for short, and 0 for flat positions
pnls = []
...
for close_price in close:
    ...
    or
    (position > 0 and (apo >= 0 or open_pnl > MIN_PROFIT_TO_CLOSE))): # long  and APO has gone positive, sell to close position
    ...
    or
    (position < 0 and (apo <= 0 or open_pnl > MIN_PROFIT_TO_CLOSE))): # short and APO has gone negative, buy to close position
