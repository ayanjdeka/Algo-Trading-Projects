...
class BaggingRegressionModel(object):
    def __init__(self):
...
lasso_reg_model = BaggingRegressionModel()
lasso_reg_model.learn(df_x, apple_prices, start_date='2020', 
                       end_date='2022', lookback_period=20)

plt.figure(figsize=(12, 8))
plt.plot(lasso_reg_model.df_result['Actual'], label= 'Apple Actual Returns')
plt.plot(lasso_reg_model.df_result['Predicted'], label = 'Strategy Returns')
plt.legend()
plt.title('Apple Returns Trained on Google Data and Using Bagging Regressor')
plt.show()
