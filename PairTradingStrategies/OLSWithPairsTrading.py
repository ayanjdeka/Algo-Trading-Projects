...
df_x = pd.DataFrame({'AAPL': aapl_data_frame['Adj Close']})
goog_prices = google_data_frame['Adj Close']

linear_reg_model = LinearRegressionModel()
linear_reg_model.learn(df_x, goog_prices, start_date='2020', 
                       end_date='2022', lookback_period=20)

plt.figure(figsize=(12, 8))
plt.plot(linear_reg_model.df_result['Actual'], label= 'GOOG Actual Returns')
plt.plot(linear_reg_model.df_result['Predicted'], label = 'Strategy Returns')
plt.legend()
plt.title('Google Returns Trained on Apple Data and Using OLS')
plt.show()
