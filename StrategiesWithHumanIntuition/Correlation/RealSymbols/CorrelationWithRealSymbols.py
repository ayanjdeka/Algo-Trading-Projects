...
plt.show()
Symbol2_prices = data['Adj Close']['QCOM']
Symbol2_prices.name = 'QCOM'
plt.title("MSFT and QCOM prices")
Symbol1_prices.plot()
Symbol2_prices.plot()
plt.legend()
plt.show()
...
'''When we have a short position, a buy order is  allowed.  When there is a long position, a sell order is allowed.
When we have no position, we can either go long or go short. For 
the paired symbol, we will do the opposite. This will limit the amount of orders that we
do, as the previous diagram has too many orders
 '''
...
#In this implementation, we hedge positions so that we do not focus entirely on 
#one position
