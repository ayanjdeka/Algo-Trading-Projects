...
for_loop_backtester = ForLoopBackTester()
for line in zip(goog_data.index, goog_data['Adj Close']):
    date = line[0]
    price = line[1]
    price_info = {'date' : date, 'price' : float(price)}
    is_tradable = for_loop_backtester.create_metrics_out_of_prices(price_info)
    if is_tradable:
        for_loop_backtester.buy_sell_or_hold_something(price_info)

plt.plot(for_loop_backtester.list_total, label = "Holdings + Cash using For Loop Backtester")
plt.legend()
plt.show()